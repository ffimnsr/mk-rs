...
impl<'input> Parser<'input> {
  ...
  pub fn next(&mut self) -> Result<(Event<'input>, Mark)> {
    ...
    unsafe {
      ...
      if parser_ref.error != sys::YAML_NO_ERROR {
...
unsafe fn convert_event<'input>(sys: &sys::yaml_event_t, input: &Cow<'input, [u8]>) -> Event<'input> {
  match sys.type_ {
    ...
    sys::YAML_ALIAS_EVENT => Event::Alias(unsafe { optional_anchor(sys.data.alias.anchor) }.unwrap()),
