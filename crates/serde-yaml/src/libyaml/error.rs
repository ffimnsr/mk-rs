...
impl Error {
  pub unsafe fn parse_error(parser: *const sys::yaml_parser_t) -> Self {
    ...
    Error {
      kind: parser_ref.error,
      problem: match NonNull::new(parser_ref.problem as *mut _) {
        Some(problem) => unsafe { CStr::from_ptr(problem) },
        None => CStr::from_bytes_with_nul(b"libyaml parser failed but there is no error\0"),
      },
      problem_offset: parser_ref.problem_offset,
      problem_mark: Mark {
        sys: parser_ref.problem_mark,
      },
      context: match NonNull::new(parser_ref.context as *mut _) {
        Some(context) => Some(unsafe { CStr::from_ptr(context) }),
        None => None,
      },
      context_mark: Mark {
        sys: parser_ref.context_mark,
      },
    }
  }

  pub unsafe fn emit_error(emitter: *const sys::yaml_emitter_t) -> Self {
    let emitter_ref = unsafe { &*emitter };

    Error {
      kind: emitter_ref.error,
      problem: match NonNull::new(emitter_ref.problem as *mut _) {
        Some(problem) => unsafe { CStr::from_ptr(problem) },
