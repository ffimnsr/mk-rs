...
impl<'de, 'document> DeserializerFromEvents<'de, 'document> {
  ...
  fn recursion_check<F: FnOnce(&mut Self) -> Result<T>, T>(&mut self, mark: Mark, f: F) -> Result<T> {
...
fn is_plain_or_tagged_literal_scalar(expected: &str, scalar: &Scalar, tagged_already: bool) -> bool {
    ...
    V: Visitor<'de>,
  {
    ...
    loop {
      match next {
        ...
        Event::Scalar(scalar) if is_plain_or_tagged_literal_scalar(Tag::BOOL, scalar, tagged_already) => {
    ...
    V: Visitor<'de>,
  {
    ...
    loop {
      match next {
        ...
        Event::Scalar(scalar) if is_plain_or_tagged_literal_scalar(Tag::INT, scalar, tagged_already) => {
          if let Ok(value) = str::from_utf8(&scalar.value) {
    ...
    V: Visitor<'de>,
  {
    ...
    loop {
      match next {
        ...
        Event::Scalar(scalar) if is_plain_or_tagged_literal_scalar(Tag::FLOAT, scalar, tagged_already) => {
          if let Ok(value) = str::from_utf8(&scalar.value) {
    ...
    V: Visitor<'de>,
  {
    ...
    match next {
      ...
      other => {
        if match other {
          Event::Void => true,
          Event::Scalar(scalar) => scalar.value.is_empty() && scalar.style == ScalarStyle::Plain,
          _ => false,
        } {
