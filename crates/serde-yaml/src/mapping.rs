  ...
  fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
    ...
    fn total_cmp(a: &Value, b: &Value) -> Ordering {
      match (a, b) {
        ...
        (Value::Mapping(a), Value::Mapping(b)) => iter_cmp_by(a, b, |(ak, av), (bk, bv)| {
          total_cmp(ak, bk).then_with(|| total_cmp(av, bv))
        }),
        (Value::Mapping(_), _) => Ordering::Less,
        (_, Value::Mapping(_)) => Ordering::Greater,

        (Value::Tagged(a), Value::Tagged(b)) => a.tag.cmp(&b.tag).then_with(|| total_cmp(&a.value, &b.value)),
      }
    }
...
impl Display for DuplicateKeyError<'_> {
  fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
    ...
    match self.entry.key() {
      ...
      Value::Sequence(_) | Value::Mapping(_) | Value::Tagged(_) => formatter.write_str("in YAML map"),
    }
  }
}
