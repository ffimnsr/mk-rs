...
impl ser::Serializer for Serializer {
  ...
  fn serialize_bytes(self, value: &[u8]) -> Result<Value> {
    let vec = value.iter().map(|&b| Value::Number(Number::from(b))).collect();
  ...
  fn serialize_unit_variant(self, _name: &str, _variant_index: u32, variant: &str) -> Result<Value> {
    ...
    V: ?Sized + ser::Serialize,
  {
    ...
    impl ser::Serializer for CheckForTag {
      ...
      fn serialize_tuple_variant(
        ...
        Ok(NotTag {
          delegate: Serializer.serialize_tuple_variant(name, variant_index, variant, len)?,
        })
      }
      ...
      fn serialize_struct_variant(
        ...
        Ok(NotTag {
          delegate: Serializer.serialize_struct_variant(name, variant_index, variant, len)?,
        })
      }
