...
main() {
    ...
    *) _bin_name="mk" ;;
    ...
    local _package
    _package="$(ensure download_mk "${_arch}")"
    ...
    *)
    ...
    ensure try_sudo cp -- "${_filename_no_ext}/README.md" "${DOC_DIR}/mk/README.md"
    echo "Installed documentation to ${DOC_DIR}"

    # Install license.
    ensure try_sudo mkdir -p -- "${LIC_DIR}/mk"
    ensure try_sudo cp -- "${_filename_no_ext}/COPYRIGHT" "${LIC_DIR}/mk/COPYRIGHT"
    ensure try_sudo cp -- "${_filename_no_ext}/LICENSE-APACHE" "${LIC_DIR}/mk/LICENSE-APACHE"
    ensure try_sudo cp -- "${_filename_no_ext}/LICENSE-MIT" "${LIC_DIR}/mk/LICENSE-MIT"
...
usage() {
    ...
    cat <<EOF
${_text_heading}mk installer${_text_reset}
Edward Fitz Abucay <gh:ffimnsr>
https://github.com/ffimnsr/mk-rs

Fetches and installs mk. If mk is already installed, it will be updated to the latest version.
...
download_mk() {
    ...
    local _releases_url="https://api.github.com/repos/ffimnsr/mk-rs/releases/latest"
    local _releases
    case "${_dld}" in
    ...
    *) err "unsupported downloader: ${_dld}" ;;
    ...
    (echo "${_releases}" | grep -q 'API rate limit exceeded') &&
        err "you have exceeded GitHub's API rate limit. Please try again later, or use a different installation method: https://github.com/ffimnsr/mk-rs/#installation"

    local _package_url
        _package_url="$(echo "${_releases}" | grep "browser_download_url" | cut -d '"' -f 4 | grep -- "${_arch}\(.tar.gz\|.zip\)$")" ||
            err "mk has not yet been packaged for your architecture (${_arch}), please file an issue: https://github.com/ffimnsr/mk-rs/issues"
    ...
    *) err "unsupported package format: ${_package_url}" ;;
    ...
    local _package="${_filename}" || "mk.${_ext}"
    case "${_dld}" in
